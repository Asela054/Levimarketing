...
$joinQuery = "FROM `tbl_porder` AS `u` LEFT JOIN `tbl_dispatch` AS `ud` ON (`ud`.`porder_id` = `u`.`idtbl_porder`) LEFT JOIN `tbl_user` AS `uc` ON (`uc`.`idtbl_user` = `u`.`tbl_user_idtbl_user`) LEFT JOIN `tbl_location` AS `l` ON (`l`.`idtbl_location` = `u`.`tbl_location_idtbl_location`) LEFT JOIN `tbl_supplier` AS `s` ON (`s`.`idtbl_supplier` = `u`.`tbl_supplier_idtbl_supplier`)";
