...
$joinQuery = "FROM `tbl_invoice` AS `i`  
              LEFT JOIN `tbl_customer` AS `c` 
                ON (`c`.`idtbl_customer` = `i`.`customerid`) 
              LEFT JOIN ( 
                SELECT 
                    `ip`.`tbl_invoice_idtbl_invoice`, 
                    SUM(`p`.`payment`) AS `total_paid` 
                FROM `tbl_invoice_payment_has_tbl_invoice` AS `ip` 
                LEFT JOIN `tbl_invoice_payment` AS `p` 
                    ON (`p`.`idtbl_invoice_payment` = `ip`.`tbl_invoice_payment_idtbl_invoice_payment`) 
                WHERE `p`.`status` = 1 
                GROUP BY `ip`.`tbl_invoice_idtbl_invoice` 
              ) AS `pm` 
                ON (`pm`.`tbl_invoice_idtbl_invoice` = `i`.`idtbl_invoice`)"; 

// Location filter
$extraWhere = "`i`.`status` IN (0,1) 
               AND `i`.`paymentmethod` = 2 
