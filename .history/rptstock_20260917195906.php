...
function applyFilters() {
    ...
    $('.category-block').each(function () {
        ...
        if (!selectedCat || selectedCat === blockCat) {
            $block.show();
            // reload only if it's already been initialized (lazy-loaded ones
            // not yet visible will pick up the filters when they DO init)
            if (tableInitialized[blockCat]) {
                stockTables[blockCat].ajax.reload();
            } else if (!selectedCat || selectedCat === blockCat) {
                // force-init immediately if a specific category is chosen
                // so the user isn't waiting on scroll to see results
