...
if ( !$loggedIn )
{
    ...
    $_SESSION['postype'] = $postype;
    $_SESSION['privatetype'] = $privatetype;
    if(isset($_POST['location']) && $_POST['location']!=''){
        $locationId = intval($_POST['location']);
        $lsql = "SELECT idtbl_location, location, code FROM tbl_location WHERE idtbl_location=$locationId LIMIT 1";
        $lres = mysqli_query($con, $lsql);
        if($lrow = mysqli_fetch_assoc($lres)){
            $_SESSION['location_id'] = $lrow['idtbl_location'];
            $_SESSION['location_name'] = $lrow['location'];
            $_SESSION['location_code'] = $lrow['code'];
        }
    }
}
?>