...
if($conn->query($insertinvoice)==true){
    ...
    // NOTE: tableDataPay row layout is now:
    // col_1 = method (hidden), col_2 = PAY METHOD text, col_3 = BANK, col_4 = CHEQUE NO,
    // col_5 = CHEQUE DATE, col_6 = CARD LAST4 (NEW), col_7 = total (hidden), col_8 = TOTAL display
    ...
    if($conn->query($insertpayment)==true){
        ...
        if(!empty($tableDataPay)){
            foreach($tableDataPay as $rowtableDataPay){
                ...
                $cardlast4=isset($rowtableDataPay['col_6']) ? $rowtableDataPay['col_6'] : '';   // NEW
                $totalamount=$rowtableDataPay['col_7'];                                          // shifted from col_6
