...
$new_qty=isset($_POST['new_qty']) ? trim($_POST['new_qty']) : '';
...
if($new_qty==='' || !is_numeric($new_qty) || floatval($new_qty)<0){
    $actionObj=new stdClass();
    $actionObj->icon='fas fa-exclamation-triangle';
    $actionObj->title='';
    $actionObj->message='Please enter a valid quantity';
    $actionObj->url='';
    $actionObj->target='_blank';
    $actionObj->type='danger';
    echo json_encode($actionObj);
...
if($reason===''){
    ...
    $actionObj->icon='fas fa-exclamation-triangle';
    $actionObj->title='';
    $actionObj->message='A reason is required to edit stock';
    $actionObj->url='';
    $actionObj->target='_blank';
    $actionObj->type='danger';
    echo json_encode($actionObj);
...
$new_qty=$conn->real_escape_string($new_qty);
...
$qty_change=floatval($new_qty)-$old_qty;
...
// ---- Stock update + audit log together, as one transaction ----
// If the log insert fails for any reason, the qty change is rolled back too.
// An edit that isn't logged must never happen.
$conn->autocommit(FALSE);

$updateStock="UPDATE `tbl_stock` SET `qty`='$new_qty', `updatedatetime`='$updatedatetime', `tbl_user_idtbl_user`='$userID'
...
if($updateOk){
    $insertLog="INSERT INTO `tbl_stock_activity_log`
        ...
        VALUES
            ('EDIT','$idtbl_stock','$productID','$productName','$stockLocationID','$locationName',
             '$old_qty','$new_qty','$qty_change','$reason',
...
if($updateOk && $insertOk){
    ...
    $actionObj->icon='fas fa-exclamation-triangle';
    $actionObj->title='';
    $actionObj->message='Record Error - nothing was updated (log write failed): '.$dbError;
    $actionObj->url='';
    $actionObj->target='_blank';
    $actionObj->type='danger';
