...
if($conn->query($insertinvoice)==true){
    ...
    foreach($tableData as $rowtabledata){
        ...
        // Stock is per product PER LOCATION (tbl_stock has one row per product/location combo),
        // so the deduction must be scoped to the location this sale is happening at — otherwise
        // it either misses the correct row or, if there happen to be multiple location rows for
        // this product, decrements all of them instead of just the one in use.
