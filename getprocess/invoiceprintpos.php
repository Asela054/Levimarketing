    ...
    <div id='DivIdToPrint'>
        <table style="width:100%;">
            ...
            <tr>
                ...
                <td style="text-align: right; font-size:14px;"><?php echo !empty($rowinvoiceinfo['manuelinvno']) ? 'INV-' .$rowinvoiceinfo['manuelinvno'] : 'INV-' . $rowinvoiceinfo['idtbl_invoice']; ?></td>
            </tr>
            <tr>
                            ...
                            ?>
                                ...
                                <tr>
                                    ...
                                    <td style="text-align: center; font-size:14px;">
                                        <?php echo ($rowinvoicedetail['editstatus'] == 1) ? '' : number_format($displayunitprice,2); ?>
                                    </td>
...
<head>
    ...
    <style media="print">
        ...
        /* Reserve the header/footer space on EVERY page (not just the first) since
           body margin only applies once at the top of the flow, not after each
           forced page break. Padding on main never collapses, so this reliably
           pushes the table below the fixed header on every page. */
        main {
            display: block;
            padding-top: 160px;
...
<body>
        ...
        <table style="width:100%;border-collapse: collapse;">
            <tr>
                ...
                <td width="45%" style="vertical-align: top;padding:0px;">
                    ...
                    <p style="margin:0px;font-size:13px;font-weight:normal;">Invoice No: INV-<?php echo htmlspecialchars(!empty($rowinvoiceinfo['manuelinvno']) ? $rowinvoiceinfo['manuelinvno'] : 'INV-' . $rowinvoiceinfo['idtbl_invoice']); ?></p>
    ...
    <footer>
        <table style="width:100%;">
            <tr>
                ...
                <td style="text-align: center;vertical-align: top;">
                    <p style="margin:0;font-size:12px;text-transform: uppercase;font-weight: bold;">Customer</p>
                    <p style="margin:0;margin-top:25px;font-size:12px;">..........................................................</p>
    ...
    <?php
        // Build the full item list first, then split into pages of 5.
        // Each page gets its OWN <table> (own thead + tbody + tfoot) instead of one
        // continuous table with forced row breaks - this is what keeps the fixed
        // header from overlapping the table on later pages, and keeps the totals
        // block from repeating on every page.
        $invoiceRows = [];
        while($rowinvoicedetail = $resultinvoicedetail->fetch_assoc()){
            $invoiceRows[] = $rowinvoicedetail;
        }

        $itemsPerPage = 6;
        $maxItemsOnTotalsPage = 3;
        $itemPages = array_chunk($invoiceRows, $itemsPerPage);
        if (empty($itemPages)) {
            $itemPages = [[]]; // still render one page (with totals) if there are no items
        } else {
            $lastIdx = count($itemPages) - 1;
            $lastChunk = $itemPages[$lastIdx];
            // The page carrying the totals section has less room for items than a
            // normal page, so cap it at $maxItemsOnTotalsPage and push any overflow
            // into an extra page just before it.
            if (count($lastChunk) > $maxItemsOnTotalsPage) {
                $splitPoint = count($lastChunk) - $maxItemsOnTotalsPage;
                $overflowPage = array_slice($lastChunk, 0, $splitPoint);
                $totalsPage   = array_slice($lastChunk, $splitPoint);
                array_splice($itemPages, $lastIdx, 1, [$overflowPage, $totalsPage]);
            }
        }
        $lastPageIndex = count($itemPages) - 1;
    ?>
    ...
    <?php foreach ($itemPages as $pageIndex => $pageRows): ?>
        ...
        <main>
            <table style="table-layout: fixed;padding:3px;width:100%;border-collapse: collapse;font-size: 13px;">
                ...
                <tbody>
                    <?php
                        $pageRowCount = count($pageRows);
                        $pageRowIndex = 0;
                        foreach ($pageRows as $rowinvoicedetail):
                        $pageRowIndex++;
                        // saleprice/editedprice are LINE AMOUNTS (qty already applied), not per-unit prices.
                        $baselineamount = ($rowinvoicedetail['editstatus']==1 && $rowinvoicedetail['editedprice'] > 0)
                            ? $rowinvoicedetail['editedprice']
                        ...
                        // Non-last pages have no <tfoot> anymore, so their last row needs to
                        // close the table border itself.
                        $isLastRowOnPage  = ($pageRowIndex === $pageRowCount);
                        $needsCloseBorder = ($isLastRowOnPage && $pageIndex !== $lastPageIndex);
                        $bottomBorder     = $needsCloseBorder ? 'border-bottom: 1px solid #000;' : '';
                    ?>
                    <tr>
                        <td style="text-align:center; border-right: 1px solid black; border-left: 1px solid #000; <?php echo $bottomBorder; ?>"><?php echo htmlspecialchars($rowinvoicedetail['product_code']); ?></td>
                        <td style="padding-left: 10px; border-right: 1px solid black; <?php echo $bottomBorder; ?>"><?php echo htmlspecialchars($rowinvoicedetail['product_name']); ?></td>
                        <td style="text-align:center; border-right: 1px solid black; <?php echo $bottomBorder; ?>"><?php echo htmlspecialchars($rowinvoicedetail['qty']); ?></td>
                        <td style="text-align:center; border-right: 1px solid black; <?php echo $bottomBorder; ?>">&nbsp;</td>
                        <td style="text-align:right; border-right: 1px solid black; padding-right: 10px; <?php echo $bottomBorder; ?>"><?php echo number_format($displayunitprice, 2); ?></td>
                        <td style="text-align:right; padding-right: 10px; border-right: 1px solid #000; <?php echo $bottomBorder; ?>"><?php echo number_format($lineTotal, 2); ?></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
