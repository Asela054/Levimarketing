...
// tbl_invoice's customer link column is `customerid`, not `tbl_customer_idtbl_customer`
// (that column doesn't exist on tbl_invoice — using it caused the query to fail,
// which is why $resultinvoice came back as `false` instead of a result object).
$sqlinvoice="SELECT `tbl_customer`.`name`, `tbl_invoice`.`date`, `tbl_invoice`.`total`, `tbl_invoice`.`idtbl_invoice` FROM `tbl_invoice` LEFT JOIN `tbl_customer` ON `tbl_customer`.`idtbl_customer`=`tbl_invoice`.`customerid` WHERE `tbl_invoice`.`customerid` IN (SELECT `idtbl_customer` FROM `tbl_customer` WHERE `type`=3 AND `status`=1) AND `tbl_invoice`.`status`=1 AND `tbl_invoice`.`chequesend`=0 ";
